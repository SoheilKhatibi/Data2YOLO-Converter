...
class DataSet():
    def __init__(self, images_path, dataset_cluster, labels_path, dataset_type):
...
def main():
    ...
    dataset_clusters = ['positive', 'negative']

    # Find source text file and image directory

    for dataset_cluster in os.listdir(src_dir):
        if (dataset_cluster not in dataset_clusters):
            print("unknown dataset cluster:", src_dataset_type)
            continue
        dataset_cluster_path = os.path.join(src_dir, dataset_cluster)
        # Iterate on source directory dataset types
        if (dataset_cluster == "positive"):
            for src_dataset_type in os.listdir(dataset_cluster_path):
                # print(src_dataset_type)
                if (src_dataset_type not in dataset_types):
                    print("unknown dataset type:", src_dataset_type)
                    continue
                src_dataset_dir = os.path.join(dataset_cluster_path, src_dataset_type)
                ...
                if os.path.isdir(src_dataset_dir):
                    ...
                    for src_dataset_type_item in os.listdir(src_dataset_dir):
                        ...
                        if src_label_found and src_img_dir_found:
                            dataset = DataSet(src_img_dir, dataset_cluster, src_label, src_dataset_type)
                            src_datasets.append(dataset)
        if (dataset_cluster == "negative"):
            # Iterate on the items in this cluster dir
            for src_dataset_type in os.listdir(dataset_cluster_path):
                src_dataset_dir = os.path.join(dataset_cluster_path, src_dataset_type)
                # Check if it is a directory (then it may be a correct dataset)
                # print(src_dataset_dir)
                if os.path.isdir(src_dataset_dir):
                    dataset = DataSet(src_dataset_dir, dataset_cluster, None, None)
                    src_datasets.append(dataset)
    ...
    for src_dataset in src_datasets:
        if (src_dataset.dataset_cluster == "positive"):
                ...
                for image in os.listdir(src_dataset.images_path):
                    ...
                    with open(image_label_path, 'r') as label_file:
                        ...
                        for line in lines:
                            ...
                            if (src_dataset.images_path == "src_dir/positive/yolo/positive-Robocup-2019/images"):
                                BALL_INDEX = 2
                            elif (src_dataset.images_path == "src_dir/positive/yolo/ball-ramtin/images"):
                                BALL_INDEX = 0
                    ...
                    data_cnt += 1
            print(data_cnt)
        elif (src_dataset.dataset_cluster == "negative"):
            for image in os.listdir(src_dataset.images_path):
                # Fill the destination text file
                dst_name = 'frame' + str(data_cnt)
                dst_file_prefix = image.split(".")[0]
